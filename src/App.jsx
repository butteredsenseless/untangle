...
function OneThingSection({ tasks, oneThing, onSet, onClear }) {
  ...
  if (isSet && !editing) {
    return (
      <div style={{background:"linear-gradient(135deg,#0d2e1a,#0f3d22)",border:"2px solid #2d8c55",borderRadius:18,padding:"16px 16px",marginBottom:16,boxShadow:"0 0 0 4px rgba(45,140,85,0.12),0 6px 24px rgba(45,140,85,0.2)",animation:"glowPulse 3s ease-in-out infinite"}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:11,fontWeight:800,color:"#4ade80",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>Today's Knot</div>
            <div style={{fontSize:15,fontWeight:800,color:"#fff",fontFamily:"'DM Sans',sans-serif",lineHeight:1.5}}>{oneThing.text}</div>
<div style={{fontSize:11,color:"rgba(255,255,255,0.35)",marginTop:4}}>{oneThing.completed?"🎉 You've untangled today's knot! The main thing is done.":"Focus here first. Everything else can wait."}</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            <button onClick={()=>setEditing(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",color:"rgba(255,255,255,0.5)",fontSize:11,fontWeight:800,cursor:"pointer",padding:"4px 8px",borderRadius:8,fontFamily:"'DM Sans',sans-serif"}}>Edit</button>
            <button onClick={onClear} style={{background:"none",border:"none",color:"rgba(255,255,255,0.2)",fontSize:15,cursor:"pointer",padding:"4px 4px",lineHeight:1}}>×</button>
          </div>
        </div>
  ...
  return (
    <div style={{background:"linear-gradient(135deg,#1a3a2a,#1e4d35)",border:"2px solid #2d6e4a",borderRadius:16,padding:"16px 16px",marginBottom:16,boxShadow:"0 4px 20px rgba(45,110,74,0.2)"}}>
      <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:13,color:"#7ed9a0",marginBottom:4}}>🎯 Today's Knot</div>
      <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",marginBottom:8}}>{isSet?"Update your focus for today:":"What's the one thing that matters most today?"}</div>
      {todayTasks.length>0&&(
        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
          {todayTasks.map(t=>(
            <button key={t.id} onClick={()=>{onSet(t.title);setEditing(false);}} style={{padding:"4px 8px",borderRadius:20,border:"2px solid rgba(126,217,160,0.3)",background:"rgba(126,217,160,0.08)",color:"#7ed9a0",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{t.title}</button>
          ))}
        </div>
      )}
      ...
      <div ref={cardRef} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}
        ...
        {isCountTask?(
          <button onClick={handleComplete} style={{width:28,height:28,minWidth:28,borderRadius:10,border:`2px solid ${area.color}`,background:colorBg(area.color),cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",marginTop:4,flexDirection:"column",padding:0}}>
            <span style={{fontSize:11,fontWeight:800,color:area.color,lineHeight:1}}>{task.dailyCount||0}</span>
            <span style={{fontSize:11,color:area.color,lineHeight:1}}>/{task.dailyTarget}</span>
          </button>
        ):(
          <button onClick={handleComplete} style={{width:26,height:26,minWidth:26,borderRadius:"50%",border:`2px solid ${area.color}`,background:task.done?area.color:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",marginTop:4,transition:"all 0.2s"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
            ...
            {recur?.id!=="none"&&<span style={{fontSize:11,background:"#f0f0ff",color:"#8B6FBE",padding:"4px 8px",borderRadius:20,fontWeight:800}}>🔁 {recur.label}{task.recurFreq>1?` ×${task.recurFreq}`:""}</span>}
          </div>
          {isCountTask&&<div style={{marginTop:4,height:4,background:"#f0f0f0",borderRadius:20,overflow:"hidden"}}><div style={{height:"100%",width:`${countProgress*100}%`,background:area.color,borderRadius:20,transition:"width 0.4s"}}/></div>}
          {task.note&&<div style={{fontSize:11,color:"#aaa",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>{task.note}</div>}
          {task.aiNudge&&<div style={{fontSize:11,color:"#8B6FBE",marginTop:4,fontFamily:"'DM Sans',sans-serif",background:"#F3EFFB",padding:"4px 8px",borderRadius:8}}>💡 {task.aiNudge}</div>}
          <div style={{display:"flex",gap:5,marginTop:4,flexWrap:"wrap"}}>
            <span onClick={()=>onEdit&&onEdit(task,'reassign')} style={{fontSize:11,background:area.bg||colorBg(area.color),color:area.color,padding:"4px 8px",borderRadius:20,fontWeight:800,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",userSelect:"none"}}>{area.emoji} {area.label} ✎</span>
            {task.deadline&&<span style={{fontSize:11,background:"#fff3e0",color:"#E09B3D",padding:"4px 8px",borderRadius:20,fontWeight:600}}>📅 {task.deadline}</span>}
            {task.recurTime&&<span style={{fontSize:11,background:"#f0f0ff",color:"#8B6FBE",padding:"4px 8px",borderRadius:20,fontWeight:600}}>🕐 {task.recurTime}</span>}
          </div>
          {task.subtasks?.length>0&&<div style={{marginTop:8,paddingLeft:4}}>{task.subtasks.map((st,i)=><div key={i} style={{fontSize:11,color:st.done?"#bbb":"#666",textDecoration:st.done?"line-through":"none",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>↳ {st.title}</div>)}</div>}
        </div>
        {!task.done&&!compact&&(
          <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"center"}}>
            <button onClick={()=>onFocus&&onFocus(task)} title="Focus timer" style={{width:28,height:28,borderRadius:8,background:"#fff9e6",border:"1.5px solid #f0d060",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>🎯</button>
            <button onClick={()=>onBreakdown&&onBreakdown(task)} title="AI Task Coach" style={{width:28,height:28,borderRadius:8,background:"#e8f5e9",border:"1.5px solid #5BAD6F44",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>🫱🍽️</button>
            <button onClick={()=>onEdit&&onEdit(task)} title="Edit" style={{width:28,height:28,borderRadius:8,background:"#f0f4ff",border:"1.5px solid #4F86C644",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>✏️</button>
          </div>
        )}
        <button onClick={()=>onDelete(task.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#ddd",fontSize:19,padding:"0 4px",marginTop:4}}>×</button>
      </div>
    </div>
  );
}
      ...
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)"}}>
        ...
        {!supported?<div style={{background:"#fff8e1",borderRadius:12,padding:16,marginBottom:16}}><p style={{fontSize:13,color:"#b8860b",fontWeight:800,margin:0}}>⚠️ Try Chrome or Safari, or paste below.</p></div>:(
          <div style={{textAlign:"center",marginBottom:16}}>
            <button onClick={toggle} style={{width:76,height:76,borderRadius:"50%",border:"none",background:listening?"#E05C3A":"linear-gradient(135deg,#3AABB5,#4F86C6)",color:"#fff",fontSize:32,cursor:"pointer",boxShadow:listening?"0 0 0 10px rgba(224,92,58,0.2)":"0 4px 20px rgba(58,171,181,0.4)",animation:listening?"mpulse 1.5s infinite":"none"}}>{listening?"⏹":"🎤"}</button>
      ...
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)"}}>
        ...
        {loading && (
          <div style={{marginTop:8,textAlign:"center"}}>
            ...
            <div style={{marginTop:8,background:"#f0f0f0",borderRadius:20,height:6}}><div style={{width:`${(progress/text.split("\n").filter(l=>l.trim()).length)*100}%`,height:"100%",background:"linear-gradient(90deg,#3AABB5,#4F86C6)",borderRadius:20,transition:"width 0.4s"}}/></div>
          </div>
        )}
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button onClick={go} disabled={!text.trim()||loading} style={{flex:1,padding:"12px",borderRadius:12,border:"none",background:text.trim()&&!loading?"linear-gradient(135deg,#3AABB5,#4F86C6)":"#e5e5e5",color:text.trim()&&!loading?"#fff":"#aaa",fontSize:15,fontWeight:800,cursor:text.trim()&&!loading?"pointer":"default",fontFamily:"'DM Sans',sans-serif"}}>{loading?"Sorting…":"Let Alexander sort these ✓"}</button>
          <button onClick={onClose} disabled={loading} style={{padding:"12px 16px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#aaa",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {areas.map(a=>(
            <button key={a.id} onClick={()=>onPick(a.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 16px",borderRadius:12,border:`2px solid ${task.area===a.id?a.color:"#e5e5e5"}`,background:task.area===a.id?(a.bg||colorBg(a.color)):"#fafafa",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
              <span style={{fontSize:19}}>{a.emoji}</span>
              <span style={{fontSize:13,fontWeight:800,color:task.area===a.id?a.color:"#555"}}>{a.label}</span>
              {task.area===a.id&&<span style={{marginLeft:"auto",fontSize:11,color:a.color,fontWeight:800}}>current</span>}
            </button>
          ))}
        </div>
        <button onClick={onClose} style={{width:"100%",marginTop:12,padding:"8px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#aaa",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
      </div>
    </div>
  );
}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:24,width:"100%",maxWidth:480}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {areas.map(a => (
              <button key={a.id} onClick={()=>setBucketId(a.id)}
                style={{padding:"8px 12px",borderRadius:20,border:`2px solid ${bucketId===a.id ? a.color : "#e5e5e5"}`,background:bucketId===a.id ? colorBg(a.color) : "#fff",color:bucketId===a.id ? a.color : "#888",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
                {a.emoji} {a.label}
              </button>
            ))}
          </div>
        ...
        <button onClick={save} disabled={!name.trim()}
          style={{width:"100%",padding:"16px",borderRadius:14,border:"none",background:name.trim() ? `linear-gradient(135deg,${selectedArea.color},${selectedArea.color}cc)` : "#e5e5e5",color:name.trim()?"#fff":"#aaa",fontSize:15,fontWeight:800,cursor:name.trim()?"pointer":"default",fontFamily:"'DM Sans',sans-serif"}}>
          {existing ? "Save changes" : "Create Project"}
        </button>
        <button onClick={onClose} style={{width:"100%",marginTop:8,padding:"8px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#aaa",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
        {existing && (
          <button onClick={() => onDelete(existing.id)}
            style={{width:"100%",marginTop:8,padding:"8px",borderRadius:12,border:"2px solid #ffdddd",background:"#fff8f8",color:"#e05c5c",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
            Delete project
          </button>
        )}
      </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:12}}><label style={lbl}>Area</label><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{areas.map(a=><button key={a.id} onClick={()=>{setArea(a.id);setProjectId(prev=>projects.find(x=>x.id===prev&&x.bucketId===a.id)?prev:null);}} style={{padding:"4px 8px",borderRadius:20,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${a.color}`,background:area===a.id?a.color:(a.bg||colorBg(a.color)),color:area===a.id?"#fff":a.color}}>{a.emoji} {a.label}</button>)}</div></div>
        {bucketProjects.length>0&&<div style={{marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><label style={lbl}>Project (optional)</label>{projectId&&projectId===aiSuggestedProjectRef.current&&<span style={{fontSize:11,color:"#3AABB5",fontWeight:800,fontFamily:"'DM Sans',sans-serif",background:"#E8F7F8",padding:"4px 8px",borderRadius:20}}>✦ Alexander suggests</span>}</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{bucketProjects.map(p=><button key={p.id} onClick={()=>setProjectId(prev=>prev===p.id?null:p.id)} style={{padding:"4px 8px",borderRadius:20,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${projectId===p.id?p.colour:"#e5e5e5"}`,background:projectId===p.id?p.colour+"22":"#fff",color:projectId===p.id?p.colour:"#aaa",fontFamily:"'DM Sans',sans-serif"}}>{p.emoji} {p.name}</button>)}</div></div>}
        <div style={{display:"flex",gap:10,marginBottom:12}}>
          <div style={{flex:1}}><label style={lbl}>When?</label>{HORIZONS.map(h=><button key={h.id} onClick={()=>setHorizon(h.id)} style={{display:"block",width:"100%",textAlign:"left",marginBottom:4,padding:"8px 8px",borderRadius:10,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${horizon===h.id?"#3AABB5":"#e5e5e5"}`,background:horizon===h.id?"#E8F7F8":"#fafafa",color:horizon===h.id?"#3AABB5":"#999",fontFamily:"'DM Sans',sans-serif"}}>{h.icon} {h.label}</button>)}</div>
          <div style={{flex:1}}><label style={lbl}>Energy</label>{ENERGY.map(en=><button key={en.id} onClick={()=>setEnergy(en.id)} style={{display:"block",width:"100%",textAlign:"left",marginBottom:4,padding:"8px 8px",borderRadius:10,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${energy===en.id?en.color:"#e5e5e5"}`,background:energy===en.id?en.color+"22":"#fafafa",color:energy===en.id?en.color:"#999",fontFamily:"'DM Sans',sans-serif"}}>{en.emoji} {en.label}</button>)}</div>
        </div>
        <div style={{background:"#f8f8f8",borderRadius:12,padding:"12px 16px",marginBottom:12}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={()=>setDailyTarget(Math.max(1,dailyTarget-1))} style={{width:28,height:28,borderRadius:8,border:"2px solid #e5e5e5",background:"#fff",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>−</button>
            <span style={{fontSize:15,fontWeight:800,color:"#333",minWidth:24,textAlign:"center",fontFamily:"'DM Sans',sans-serif"}}>{dailyTarget}</span>
            <button onClick={()=>setDailyTarget(dailyTarget+1)} style={{width:28,height:28,borderRadius:8,border:"2px solid #e5e5e5",background:"#fff",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>+</button>
            <span style={{fontSize:11,color:"#aaa"}}>{dailyTarget===1?"once (standard)":"per day"}</span>
          </div>
        </div>
        <div style={{marginBottom:12}}>
          <label style={lbl}>Repeats?</label>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>{RECUR_OPTIONS.map(r=><button key={r.id} onClick={()=>setRecur(r.id)} style={{padding:"4px 8px",borderRadius:20,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${recur===r.id?"#8B6FBE":"#e5e5e5"}`,background:recur===r.id?"#F3EFFB":"#fafafa",color:recur===r.id?"#8B6FBE":"#999",fontFamily:"'DM Sans',sans-serif"}}>{r.id!=="none"?"🔁 ":""}{r.label}</button>)}</div>
          {recur==="weekly"&&(
            <div style={{background:"#f8f8f8",borderRadius:12,padding:"8px 12px",marginBottom:8}}>
              <p style={{fontSize:11,color:"#aaa",margin:"0 0 8px"}}>Which days?</p>
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{DAYS_OF_WEEK.map((d,i)=><button key={i} onClick={()=>toggleDay(i)} style={{padding:"4px 8px",borderRadius:8,fontSize:11,fontWeight:800,cursor:"pointer",border:`2px solid ${recurDays.includes(i)?"#8B6FBE":"#e5e5e5"}`,background:recurDays.includes(i)?"#F3EFFB":"#fff",color:recurDays.includes(i)?"#8B6FBE":"#aaa"}}>{d}</button>)}</div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:11,color:"#aaa"}}>Times per week:</span>
                <button onClick={()=>setRecurFreq(Math.max(1,recurFreq-1))} style={{width:24,height:24,borderRadius:6,border:"2px solid #e5e5e5",background:"#fff",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>−</button>
                <span style={{fontSize:15,fontWeight:800,color:"#333",fontFamily:"'DM Sans',sans-serif"}}>{recurFreq}</span>
                <button onClick={()=>setRecurFreq(recurFreq+1)} style={{width:24,height:24,borderRadius:6,border:"2px solid #e5e5e5",background:"#fff",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>+</button>
              </div>
            </div>
          )}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&(onSave(text),onClose())}>
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        ...
        <textarea value={text} onChange={e=>setText(e.target.value)} autoFocus placeholder={"Packing for trip:\n- passport\n- charger\n\nThings to Google:\n- best coffee Oxford"} style={{flex:1,minHeight:280,padding:"12px 16px",borderRadius:12,border:"2px solid #e5e5e5",fontSize:15,fontFamily:"'DM Sans',sans-serif",outline:"none",resize:"none",lineHeight:1.5}}/>
      </div>
    </div>
  );
}
      ...
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{background:"#fafafa",borderRadius:14,padding:"16px 16px",marginBottom:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={()=>logoRef.current?.click()} style={{padding:"8px 16px",borderRadius:11,border:"2px solid #e5e5e5",background:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",color:"#555"}}>Upload logo</button>
            {appLogo&&<button onClick={()=>setAppLogo(null)} style={{padding:"8px 8px",borderRadius:11,border:"2px solid #fee",background:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",color:"#E05C3A"}}>Remove</button>}
        ...
        <div style={{background:"#fafafa",borderRadius:14,padding:"16px 16px",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            {[{id:"morning",label:"☀️ Morning"},{id:"evening",label:"🌙 Evening"}].map(p=>(
              <button key={p.id} onClick={()=>setPlanningPref(p.id)} style={{flex:1,padding:"8px",borderRadius:11,border:`2px solid ${planningPref===p.id?"#3AABB5":"#e5e5e5"}`,background:planningPref===p.id?"#e8f7f8":"#fff",color:planningPref===p.id?"#3AABB5":"#888",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{p.label}</button>
            ))}
          </div>
        ...
        <button onClick={onClose} style={{width:"100%",padding:"12px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#888",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginTop:8}}>Done ✓</button>
      </div>
    </div>
  );
}
...
function FocusTimer({ task, areas, onDone, onComplete, soundEnabled }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#080e12",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:"center",maxWidth:340,padding:24,width:"100%"}}>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.3)",fontWeight:800,letterSpacing:"0.15em",marginBottom:8}}>FOCUS MODE</div>
        <div style={{fontSize:19,color:"#fff",fontWeight:800,marginBottom:24,lineHeight:1.2,padding:"0 24px"}}>{task?.title}</div>
        <div style={{position:"relative",width:200,height:200,margin:"0 auto 32px"}}>
          ...
          <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <span style={{fontSize:44,fontWeight:800,color:finished?"#5BAD6F":"#fff",letterSpacing:"-2px"}}>{finished?"✓":`${m}:${s}`}</span>
          </div>
        </div>
        {!finished?(
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
            <button onClick={()=>setRunning(r=>!r)} style={{padding:"12px 32px",borderRadius:14,border:"none",background:area.color,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>{running?"⏸ Pause":"▶ Start"}</button>
            <button onClick={onDone} style={{padding:"12px 16px",borderRadius:14,border:"2px solid rgba(255,255,255,0.1)",background:"transparent",color:"rgba(255,255,255,0.4)",fontSize:15,fontWeight:800,cursor:"pointer"}}>Exit</button>
          </div>
        ):(
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
            <button onClick={()=>{onComplete(task.id);onDone();}} style={{padding:"12px 24px",borderRadius:14,border:"none",background:"#5BAD6F",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>✓ Mark done!</button>
            <button onClick={onDone} style={{padding:"12px 16px",borderRadius:14,border:"2px solid rgba(255,255,255,0.1)",background:"transparent",color:"rgba(255,255,255,0.4)",fontSize:15,fontWeight:800,cursor:"pointer"}}>Not done yet</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.16)"}}>
        ...
        {!loading&&subtasks.length>0&&(<>
          {subtasks.map((st,i)=><div key={i} style={{display:"flex",gap:8,padding:"8px 8px",background:"#fafafa",borderRadius:10,marginBottom:4}}><span style={{color:"#ccc",fontSize:13}}>{i+1}.</span><span style={{fontSize:15,fontFamily:"'DM Sans',sans-serif",color:"#333"}}>{st.title}</span></div>)}
          <div style={{display:"flex",gap:8,marginTop:16}}>
            <button onClick={()=>onSave(task.id,subtasks)} style={{flex:1,padding:"12px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#3AABB5,#4F86C6)",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Save subtasks ✓</button>
            <button onClick={onClose} style={{padding:"12px 16px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#aaa",fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
          </div>
        </>)}
  ...
  return (
    <div>
      <div style={{display:"flex",gap:5,marginBottom:12}}>
        {["day","week","month"].map(m=><button key={m} onClick={()=>{setMode(m);setOffset(0);}} style={{padding:"8px 16px",borderRadius:10,border:`2px solid ${mode===m?"#3AABB5":"#e0e0e0"}`,background:mode===m?"#E8F7F8":"#fff",color:mode===m?"#3AABB5":"#999",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",textTransform:"capitalize"}}>{m}</button>)}
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",borderRadius:14,padding:"8px 16px",marginBottom:16,boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
        <button onClick={()=>setOffset(o=>o-1)} style={{width:32,height:32,borderRadius:8,border:"2px solid #e0e0e0",background:"#fff",cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>←</button>
        <div style={{textAlign:"center"}}><div style={{fontWeight:800,fontSize:15,color:"#1a1a1a",fontFamily:"'DM Sans',sans-serif"}}>{range.label}</div><div style={{fontSize:11,color:"#aaa"}}>{visibleTasks.length} tasks</div></div>
        <button onClick={()=>setOffset(o=>o+1)} style={{width:32,height:32,borderRadius:8,border:"2px solid #e0e0e0",background:"#fff",cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>→</button>
      </div>
      {offset!==0&&<button onClick={()=>setOffset(0)} style={{width:"100%",padding:"8px",borderRadius:10,border:"2px solid #e0e0e0",background:"#fff",color:"#aaa",fontSize:11,fontWeight:800,cursor:"pointer",marginBottom:12,fontFamily:"'DM Sans',sans-serif"}}>Back to current {mode}</button>}
      {["today","week","month"].filter(h=>mode==="day"?h==="today":mode==="week"?h!=="month":true).map(h=>{ const group=visibleTasks.filter(t=>t.horizon===h); if(!group.length)return null; const hor=HORIZONS.find(x=>x.id===h); return(<div key={h} style={{marginBottom:24}}><h3 style={{fontSize:11,fontWeight:800,color:"#bbb",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8,fontFamily:"'DM Sans',sans-serif"}}>{hor.icon} {hor.label} · {group.length}</h3>{group.map(task=><TaskCard key={task.id} task={task} areas={areas} onComplete={onComplete} onDelete={onDelete} onBreakdown={onBreakdown} onFocus={onFocus} onEdit={onEdit} soundEnabled={soundEnabled}/>)}</div>); })}
      {visibleTasks.length===0&&<div style={{textAlign:"center",padding:"48px 24px",color:"#bbb"}}><div style={{fontSize:38}}>✨</div><p style={{fontWeight:800,fontFamily:"'DM Sans',sans-serif"}}>Nothing here</p></div>}
    </div>
  );
}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,25,35,0.85)",zIndex:1200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      ...
      <div style={{background:"#f7f8fa",borderRadius:22,width:"100%",maxWidth:480,maxHeight:"88vh",display:"flex",flexDirection:"column",boxShadow:"0 24px 64px rgba(0,0,0,0.16)",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 16px 12px",borderBottom:"1px solid #ebebeb",background:"#fff"}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:19,cursor:"pointer",color:"#bbb",lineHeight:1}}>×</button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:12}}>
          {messages.map((msg, i) => (
            <div key={i} style={{animation:"fadein 0.3s ease",display:"flex",justifyContent:msg.type==="user"?"flex-end":"flex-start",alignItems:"flex-end",gap:8}}>
              {msg.type==="alex"&&<div style={{fontSize:19,marginBottom:4}}>🧠</div>}
              <div style={bubbleStyle(msg.type)}>{msg.text}</div>
            </div>
          ))}
          {loading&&<div style={{animation:"fadein 0.3s ease",display:"flex",alignItems:"flex-end",gap:8}}><div style={{fontSize:19}}>🧠</div><div style={{...bubbleStyle("alex"),color:"#3AABB5"}}>Thinking…</div></div>}
          <div ref={bottomRef}/>
        </div>
        ...
        <div style={{padding:"12px 16px 16px",background:"#fff",borderTop:"1px solid #ebebeb"}}>
          {phase==="focus"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {projects.map(p=>(
                    <button key={p.id} onClick={()=>handleFocusPick(p.id,`${p.emoji} ${p.name}`)}
                      style={{padding:"8px 16px",borderRadius:20,border:"2px solid #e5e5e5",background:"#fff",color:"#333",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>
                      {p.emoji} {p.name}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={()=>handleFocusPick("all","Mix of everything")}
                style={{padding:"12px 16px",borderRadius:12,border:"2px solid #e5e5e5",background:"#fff",color:"#333",fontSize:15,fontWeight:800,cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif"}}>
                Mix of everything
              </button>
            </div>
          )}
          ...
          {phase==="empty"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <button onClick={()=>handleFocusPick("all","Mix of everything")}
                style={{padding:"12px 16px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#3AABB5,#4F86C6)",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif"}}>
                Mix of everything
              </button>
            </div>
          )}
          ...
          {phase==="chips"&&(
            <div>
                ...
                {suggested.map(id=>{
                  ...
                  return (
                    <button key={id} onClick={()=>setSelected(prev=>{const next=new Set(prev);isOn?next.delete(id):next.add(id);return next;})}
                      style={{padding:"8px 12px",borderRadius:20,fontSize:11,fontWeight:800,cursor:"pointer",
              ...
              {showOverwhelmPrompt&&(
                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  <button onClick={onEscape} style={{flex:1,padding:"8px",borderRadius:11,border:"none",background:"#3AABB5",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Yes, brain dump</button>
                  <button onClick={()=>setShowOverwhelmPrompt(false)} style={{flex:1,padding:"8px",borderRadius:11,border:"2px solid #e5e5e5",background:"#fff",color:"#555",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>I'm ok, carry on</button>
                </div>
              )}
              {/* Free text add */}
              {!showOverwhelmPrompt&&(
              ...
              <button onClick={handleConfirm} style={{width:"100%",padding:"12px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#3AABB5,#4F86C6)",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
                That's my day →
              </button>
            </div>
          )}
          ...
          {(phase==="focus"||phase==="chips"||phase==="empty")&&(
            <button onClick={onEscape} style={{width:"100%",marginTop:8,padding:"8px",borderRadius:10,border:"1px solid #e5e5e5",background:"transparent",color:"#bbb",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
...
const lbl={display:"block",fontSize:11,fontWeight:800,color:"#bbb",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8,fontFamily:"'DM Sans',sans-serif"};
...
const setOneThingFn=useCallback(text=>{const val={text,date:todayStr()};setOneThing(val);oneThingRef.current=val;},[]);  const incomplete=tasks.filter(t=>!t.done);
  ...
  setReassignTask(null);
      ...
      <div style={{background:"linear-gradient(160deg,#0f1923 0%,#152232 60%,#1a2d3a 100%)",padding:"16px 24px 0",boxShadow:"0 4px 24px rgba(0,0,0,0.2)"}}>
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:11}}>
              ...
              <div>
                <div style={{color:"#fff",fontSize:19,fontWeight:800,letterSpacing:"-0.5px"}}>{appName}</div>
                <div style={{color:"rgba(255,255,255,0.35)",fontSize:11,fontWeight:600}}>{incomplete.length} tasks to untangle{streak>0?` · 🔥 ${streak} days`:""}{isCurrentlyAway&&awayMode.to?` · ✈ Away until ${new Date(awayMode.to+"T00:00:00").toLocaleDateString("en-GB",{day:"numeric",month:"short"})}`:""}</div>
              </div>
            </div>
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              <button onClick={()=>setShowNotes(true)} title="Quick notes" style={{width:36,height:36,borderRadius:10,border:"2px solid rgba(255,255,255,0.12)",background:"transparent",color:"rgba(255,255,255,0.5)",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>📝</button>
          ...
          <div style={{display:"flex",gap:5,marginBottom:12,flexWrap:"wrap"}}>
            {[{label:"🧠 Dump",action:()=>setShowDump(true)},{label:"🎤 Voice",action:()=>setShowVoice(true)},].map(b=>(
              <button key={b.label} onClick={b.action} style={{padding:"4px 12px",borderRadius:20,border:"2px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.7)",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>{b.label}</button>
            ))}
            {todayPlanDate!==todayStr()&&!isCurrentlyAway&&<button onClick={()=>setShowPlanModal(true)} style={{padding:"4px 12px",borderRadius:20,border:"2px solid rgba(58,171,181,0.4)",background:"rgba(58,171,181,0.12)",color:"rgba(255,255,255,0.85)",fontSize:11,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>Plan day ✦</button>}
          </div>
          <div style={{display:"flex",gap:2,overflowX:"auto"}}>
            {[{id:"today",label:"📋 Today"},{id:"calendar",label:"📅 Calendar"},{id:"area",label:"🗂️ By Area"},{id:"stats",label:"📊 Progress"}].map(v=>(
              <button key={v.id} onClick={()=>setView(v.id)} style={{padding:"8px 16px",borderRadius:"12px 12px 0 0",fontSize:11,fontWeight:800,cursor:"pointer",border:"none",whiteSpace:"nowrap",background:view===v.id?"#F2F1EF":"transparent",color:view===v.id?"#152232":"rgba(255,255,255,0.5)",flexShrink:0}}>{v.label}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"24px 16px"}}>
        {view==="today"&&(
          <div>
            ...
            {showWelcomeBack&&(
              <div style={{background:"linear-gradient(135deg,#3AABB5,#4F86C6)",borderRadius:16,padding:"16px 16px",marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,0.08)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <div style={{fontSize:15,fontWeight:800,color:"#fff",fontFamily:"'DM Sans',sans-serif"}}>Welcome back — want to see what's waiting?</div>
                  <div style={{fontSize:11,color:"rgba(255,255,255,0.75)",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>Alexander can pick up where you left off.</div>
                </div>
                <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0,marginLeft:12}}>
                  <button onClick={()=>{setShowWelcomeBack(false);setShowPlanModal(true);}} style={{padding:"8px 16px",borderRadius:11,border:"2px solid rgba(255,255,255,0.5)",background:"rgba(255,255,255,0.15)",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>Plan my day</button>
                  <button onClick={()=>setShowWelcomeBack(false)} style={{background:"none",border:"none",color:"rgba(255,255,255,0.5)",fontSize:19,cursor:"pointer",lineHeight:1,padding:"4px"}}>×</button>
                </div>
              </div>
            )}
            {todayPlanDate!==todayStr()&&!isCurrentlyAway&&(
              <div style={{background:"#fff",borderRadius:16,padding:"16px 16px",marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,0.05)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <div style={{fontSize:15,fontWeight:800,color:"#333",fontFamily:"'DM Sans',sans-serif"}}>Want to plan your day?</div>
                  <div style={{fontSize:11,color:"#aaa",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>Alexander can pick 3–5 tasks to focus on.</div>
                </div>
                <button onClick={()=>setShowPlanModal(true)} style={{padding:"8px 16px",borderRadius:11,border:"none",background:"linear-gradient(135deg,#3AABB5,#4F86C6)",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>Plan day ✦</button>
              </div>
            )}
            {(()=>{ const todayTasks=incomplete.filter(t=>t.horizon==="today"); return todayTasks.length===0?(<div style={{textAlign:"center",padding:"32px 24px",color:"#bbb"}}><div style={{fontSize:44}}>📋</div><p style={{fontWeight:800,fontSize:15,fontFamily:"'DM Sans',sans-serif"}}>{todayPlanDate===todayStr()?"All done for today!":"No tasks planned yet"}</p></div>):todayTasks.map(task=><TaskCard key={task.id} task={task} areas={areas} onComplete={completeTask} onDelete={deleteTask} onBreakdown={setBreakdownTask} onFocus={setFocusTask} onEdit={(t,mode)=>mode==='reassign'?setReassignTask(t):setEditTask(t)} soundEnabled={soundEnabled}/>); })()}
            {done.filter(t=>t.lastCountDate===todayStr()).length>0&&(
              <div style={{marginTop:24}}>
                <div style={{fontSize:11,fontWeight:800,color:"#bbb",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8,fontFamily:"'DM Sans',sans-serif"}}>Done today</div>
...
{view==="calendar"&&<CalendarView tasks={tasks} areas={areas} onComplete={completeTask} onDelete={deleteTask} onBreakdown={setBreakdownTask} onFocus={setFocusTask} onEdit={(task, mode)=>mode==='reassign'?setReassignTask(task):setEditTask(task)} soundEnabled={soundEnabled}/>}
        {view==="area"&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:7,marginBottom:16}}>
              {areas.map(a=>{ const n=incomplete.filter(t=>t.area===a.id).length; const pn=projects.filter(p=>p.bucketId===a.id).length; return <button key={a.id} onClick={()=>setActiveArea(a.id)} style={{padding:"8px 8px",borderRadius:14,border:`2px solid ${activeArea===a.id?a.color:"#e0e0e0"}`,background:activeArea===a.id?(a.bg||colorBg(a.color)):"#fff",textAlign:"center",cursor:"pointer",boxShadow:activeArea===a.id?`0 4px 14px ${a.color}33`:"none",transition:"all 0.18s"}}><div style={{fontSize:19}}>{a.emoji}</div><div style={{fontSize:11,fontWeight:800,color:activeArea===a.id?a.color:"#aaa",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>{a.label}</div>{n>0&&<div style={{fontSize:11,color:a.color,fontWeight:800,marginTop:4}}>{n}</div>}{pn>0&&<div style={{fontSize:11,color:"#aaa",fontWeight:800,marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>📁 {pn}</div>}</button>; })}
            </div>
        ...
        {view!=="stats"&&(
          <div style={{marginTop:24,background:"#fff",borderRadius:14,padding:"8px 16px",display:"flex",gap:10,flexWrap:"wrap",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            {areas.map(a=>{ const n=incomplete.filter(t=>t.area===a.id).length; if(!n)return null; return <div key={a.id} style={{display:"flex",alignItems:"center",gap:3}}><span style={{fontSize:13}}>{a.emoji}</span><span style={{fontSize:11,fontWeight:800,color:a.color,fontFamily:"'DM Sans',sans-serif"}}>{n}</span></div>; })}
            {incomplete.length===0&&<span style={{fontSize:11,color:"#ccc",fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>All untangled! 🌟</span>}
          </div>
        )}
      </div>
    </div>
